...
public class NVQLAttrDefList{
    ...
    private LinkedHashMap<String, String> attrDefs = new LinkedHashMap<>(); // Attribute definition list is implemented as a hash map
    
    private String attrDefSeparator = ","; // Attribute definitions are separated by ',' 
    private String attrNameTypeSeparator = ":"; // Individual attribute definitions consists of attribute name and it's data type separated by a ':'
    ...
    NVQLAttrDefList(String attrDefList){
        ...
        StringTokenizer st = new StringTokenizer(attrDefList, this.attrDefSeparator); //Separete the individual attribute definitions
        // For each such attribute definition
    ...
    // Converts the attribute definition list to a form suitable for use in an SQL query
    ...
    String convertADL4SQL(){
        ...
        for (String key : attrDefs.keySet()){
            ...
            switch(attrDefs.get(key)){
                case "string" : list = list + "TEXT" + ","; break;
                case "int" : list = list + "INT" + ","; break;
                case "time" : list = list + "TIME" + ","; break;
            } 
        }
        if (list.length()>0)
    ...
    public static void main(String args[]){
        //NVQLAttrDefList adl = new NVQLAttrDefList("name :         string,  ipAddr        : string            , mount : int"); 
        ...
        for (int i =0; i< adl.getAttrList().size();i++){
            System.out.println(adl.getAttrList().get(i)+":"+adl.getAttrDataType(adl.getAttrList().get(i)));
        }
